...
def fetch_wnba_players():
    # ESPN hidden web API endpoint for WNBA stats by athlete
    ...
    params = {
        ...
        "limit": "300"      # Ensures we grab all active players across the league
    }
    ...
    for entry in athletes:
        ...
        # Extract Points Per Game (PPG) from stats array
        ppg = 0.0
...
def save_to_js(players, filename="players.js"):
    # Sort players by PPG descending (highest scorers first)
    players_sorted = sorted(players, key=lambda x: x['ppg'], reverse=True)
    
    # Format as a JavaScript array file so your frontend can read it directly
...
if __name__ == "__main__":
    try:
        ...
        if players:
            ...
            print("Warning: No players returned from the API.")
            exit(1)
